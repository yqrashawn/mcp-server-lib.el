...
(defun httpd/mcp/v1/messages (proc uri-path uri-query request)
  "Handle POST requests to /mcp/v1/messages endpoint.
PROC is the process, URI-PATH is the path, URI-QUERY is query params, REQUEST is headers."
  ...
  (let* ((method (caar request))  ; First element is (METHOD PATH VERSION)
    ...
    (cond
     ...
     ((string= method "OPTIONS")
      (mcp-server-lib-http--set-cors-headers proc)
      (with-temp-buffer
     ...
     ((string= method "POST")
      (if (string-empty-p body)
        ...
        (condition-case err
            (let* ((json-request (json-read-from-string body))
                   (response (mcp-server-lib-process-jsonrpc body)))
