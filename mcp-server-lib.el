...
;; Keywords: comm, tools
;; Version: 0.2.0
;; Package-Requires: ((emacs "29.1") (gptel "0.9") (dash "2.20.0") (lgr "0") (simple-httpd "1.6") (compat "31"))
...
(defmacro mcp-server-lib--json-read ()
  "Read JSON from buffer using gptel's semantics."
...
(defun mcp-server-lib--convert-gptel-args-to-schema (gptel-args)
  "Convert gptel tool args to keyword-keyed plist schema (gptel's own format).
Converts from: ((:name \"location\" :type string :description \"...\"))
To: (:type \"object\" :properties (:location (:type \"string\" :description \"...\")))"
  (if (null gptel-args)
      '(:type "object" :properties nil)
    (let ((properties-plist '())
          (required '()))
      ;; Process each arg
      (dolist (arg gptel-args)
        (let* ((name (plist-get arg :name))
               ...
               (type-string (if (stringp type) type (symbol-name type)))
               ;; Create keyword from name string
               (prop-key (intern (concat ":" name)))
               ;; Build property schema
               (prop-schema (list :type type-string)))

          (when desc
            (setq prop-schema (plist-put prop-schema :description desc)))
          (when enum
            (setq prop-schema (plist-put prop-schema :enum enum)))
          (when items
            (setq prop-schema (plist-put prop-schema :items
                                         (mcp-server-lib--convert-schema-plist items))))

          ;; Add to properties plist (keyword-keyed!)
          (setq properties-plist (plist-put properties-plist prop-key prop-schema))

          ;; Add to required list if not optional
          (unless optional
            (push name required))))

      ;; Return keyword-keyed plist that json-serialize will encode correctly
      (list :type "object"
            :properties properties-plist
            :required (vconcat (nreverse required))
            :additionalProperties :json-false))))

(defun mcp-server-lib--convert-schema-plist (schema-plist)
  "Convert nested schema plist (like :items) to keyword-keyed format.
Handles nested object types with :properties."
  (let ((result '())
        (type (plist-get schema-plist :type))
        (desc (plist-get schema-plist :description))
        (props (plist-get schema-plist :properties))
        (req (plist-get schema-plist :required))
        (items (plist-get schema-plist :items)))

    (when type
      (let ((type-string (if (stringp type) type (symbol-name type))))
        (setq result (plist-put result :type type-string))))
    (when desc
      (setq result (plist-put result :description desc)))
    (when items
      (setq result (plist-put result :items
                              (mcp-server-lib--convert-schema-plist items))))

    ;; Convert :properties from symbol-keyed to keyword-keyed
    (when props
      (let ((kw-props '()))
        (while props
          (let* ((prop-name (car props))
                 (prop-value (cadr props))
                 (prop-key (if (keywordp prop-name)
                               prop-name
                             (intern (concat ":" (symbol-name prop-name))))))
            (setq kw-props (plist-put kw-props prop-key
                                      (mcp-server-lib--convert-schema-plist prop-value)))
            (setq props (cddr props))))
        (setq result (plist-put result :properties kw-props))))

    (when req
      (setq result (plist-put result :required (if (vectorp req) req (vconcat req)))))

    (when (equal (plist-get result :type) "object")
      (setq result (plist-put result :additionalProperties :json-false)))
...
Optional properties:
  ...
  :args            List of argument specs in gptel format (see below)

The :args property should be a list of plists, where each plist describes
one argument with these keys:
  :name            Argument name (string)
  :type            Argument type (symbol: string, number, integer, etc.)
  :description     Description of the argument (string)
...
Example:
  ...
  (mcp-server-lib-register-tool #\\='my-org-files-handler
    :id \"org-list-files\"
    :description \"Lists all available Org mode files\")

  ;; Tool with arguments using gptel schema format
...
See also:
  ...
  `mcp-server-lib-register-gptel-tool' - Register gptel-tool directly
  ...
  (let* ((id (plist-get properties :id))
    ...
    (when (gethash id mcp-server-lib--tools)
      (mcp-server-lib-unregister-tool id)
      (mcp-server-lib-unregister-tool id))
    (if-let* ((existing (gethash id mcp-server-lib--tools)))
      (mcp-server-lib--ref-counted-register
       id existing mcp-server-lib--tools)
      (let* ((schema
              (mcp-server-lib--convert-gptel-args-to-schema args)
              ;; (mcp-server-lib--convert-plist-schema-to-alist args)
              )
             (tool
              (list
               :id id
...
(defun mcp-server-lib-register-gptel-tool (gptel-tool)
  "Register a GPTEL-TOOL as an MCP tool.

GPTEL-TOOL should be a gptel-tool struct created with `gptel-make-tool'.
This function converts the gptel tool specification to MCP format and
registers it using `mcp-server-lib-register-tool'.

The gptel tool's function will be wrapped to handle async execution
if the tool is marked as async.

Example:
  (require 'gptel)
  (gptel-make-tool
   :function (lambda (location) (format \"Weather in %s\" location))
   :name \"get_weather\"
...
See also:
  `mcp-server-lib-register-tool' - Register tool with explicit properties
  `gptel-make-tool' - Create a gptel tool
  `gptel-get-tool' - Retrieve a registered gptel tool"
